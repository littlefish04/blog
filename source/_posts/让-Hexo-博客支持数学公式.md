...
- **不要用 `\$` 在公式里表示字面量 `$`**：`hexo-renderer-marked` 会把 `\$` 的反斜杠吃掉、只留下 `$`，残留的 `$` 会和相邻的 `$` 配对成空公式，导致该行的公式与正文**整体错位**——本该按公式渲染的 `Imm`、`r_a` 变成普通文本，而本该是正文的顿号、逗号反而被渲染成公式。要显示 `$` 本身，请改用行内代码，例如把立即数寻址写成反引号包裹的 `$Imm`。
- **`$$...$$` 块内部不要留空行**：空行是 Markdown 的段落分隔符，会把公式块拆成多个 `<p>`，`hexo-filter-mathjax` 就找不到成对的 `$$`，整块原样输出成 `\begin{aligned}` 之类的原始文本。这同样只在网站上出问题——Typora 自己的解析器容忍块内空行，本地预览一切正常。
